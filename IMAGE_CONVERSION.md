...
6. Replace the draft with the trace and compare it on the basemap. Confirm the review before saving. **Reverse route** reverses the exported direction as well as the visible start/end labels.
...
Processing is local; neither the image nor calibration is sent to Kimi. Automatic location recognition remains future work. The public page never reads `.env` or contains API keys.
