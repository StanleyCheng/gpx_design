...
The bottom panel starts minimized on every page load and shows only a handle; tap or click it to open the controls. “Put pins on the map” is selected by default; adding pins still begins with Start adding pins. It has five stages: Adding Pins, the chosen input tool, Requirements, Routes and Export. Selecting a method opens its matching input tab; accepted points advance to Requirements. Tabs preserve entered data, and later stages become available when their required data exists. Planning status sits beside the logo, with Find routes directly below the header on the right. Instructions and information cards are hidden inside the panel; feedback appears above it, and selected-route evidence opens separately through Details. Export safeguards remain active.
...
The planning map stays clear when empty. Choose Adding Pins → Put pins on the map → Start adding pins. On desktop, drag to pan and use the wheel or trackpad to zoom; right-click a pin to remove it. On iPhone, drag the map with one finger, pinch with two, and tap a pin for Delete. Drag a pin to move it. Open the panel again to finish adding pins or review their order and route limits. Tabs support arrow-key, Home and End navigation; the panel scrolls independently and respects the phone’s bottom safe area.
