...
The bottom panel has five stages: Adding Pins, the chosen input tool, Requirements, Routes and Export. Selecting a method opens its matching input tab; accepted points advance to Requirements. Tabs preserve entered data, and later stages become available when their required data exists. The minimized panel shows only a handle. Planning status sits beside the logo, with Find routes directly below the header on the right. Instructions and information cards are hidden inside the panel; feedback appears above it, and selected-route evidence opens separately through Details. Export safeguards remain active.
