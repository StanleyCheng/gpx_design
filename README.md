...
**Loop** in Requirements is off by default. When enabled, every candidate returns to the exact same mapped start/finish coordinate near the same service-linked transport stop. That stop must support both arrival and departure. Common stops with both walking approaches within 1 km are preferred; if none qualify, both end searches can expand to 20 km. The full return walk counts toward distance and road limits. Every pin remains mandatory, and the first option keeps pin order. Reordered alternatives must also close at the same stop. A loop may retrace paths, including an out-and-back; it does not promise a circuit with no repeated sections. If no mapped return or shared stop qualifies, the app explains the failure and never adds an artificial closing line.

The engine returns fewer options or an actionable failure instead of inventing routes or silently relaxing limits. The first option searches by actual walking metres in pin order. Alternatives prefer hiking relations, clear tagged paths and nearby official trail corridors, penalise roads and retracing, and may reorder up to 8 pins only for a shorter walk or better mapped-trail score. Strict order remains selectable. These preferences are not current-condition or safety assessments. It is a bounded heuristic, not a globally optimal itinerary or timetable planner. Transport is sought within a 1 km walking approach at each end first. If an end lacks a reachable service-linked stop, only that end’s map search expands progressively to 4 km, 10 km and at most 20 km; the other end keeps its nearby-stop preference. The first successful bounded search stops further downloads. Both approaches, plus the disclosed stop gaps, count toward the hike limit. The 20 km bound is checked on walking distance, not just a straight-line circle. A 30 m default waypoint tolerance is explicit, reviewable and adjustable. For each waypoint, snapping considers up to 24 deterministic eligible segment candidates within that exact tolerance, reserving candidates from distinct path clusters. A layered directed search chooses the connected pin-order combination with the smallest total snap offset; mapped walking distance and stable segment identity break ties. One-way foot rules remain active. Visually crossing ways stay disconnected unless OSM supplies a shared node, and no synthetic connector fills any waypoint or transport-stop gap.
