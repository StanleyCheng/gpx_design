    ...
    const guideScroll = (id, focusId = id) => { $(id)?.scrollIntoView({ behavior: 'smooth', block: 'start' }); setTimeout(() => $(focusId)?.focus({ preventScroll: true }), 350); };
    function guideEndpointReady() { try { return !!recognitionEndpoint(); } catch { return false; } }
    function guideModel() {
      return {
        method: $('input-method').value, hasCoordinatesText: !!$('coordinates').value.trim(), hasImage: !!ai.image,
    ...
    function updateInputGuide() {
      ...
      guideMethodOptions.forEach(option => { option.textContent = copy.methods[option.value].label; });
