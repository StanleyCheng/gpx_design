    ...
    function showRoutes() {
      ...
      routing.result.routes.forEach((route, i) => {
        ...
        if (route.start.extendedApproach || route.end.extendedApproach) facts.append(element('li', `Extended transport walk: ${[route.start.extendedApproach ? 'start' : '', route.end.extendedApproach ? 'finish' : ''].filter(Boolean).join(' and ')}. No qualifying 1 km approach was found at that end; the 10 km limit applies.`));
    ...
    $('find-routes').addEventListener('click', async () => {
      ...
      try {
        ...
        const settings = { radius, maxApproach: 1000, maxDistance: +$('plan-distance').value, maxRoad: +$('plan-roads').value, tolerance: +$('plan-tolerance').value, optimize: $('plan-order').value === 'optimize' };
        ...
        try {
          ...
          try { result = await workerPlan(mapData.data, points, settings, official.features, serial); }
            ...
            toast('No transport within a 1 km walk at an end. Checking connected approaches up to 10 km…', false, 0);
