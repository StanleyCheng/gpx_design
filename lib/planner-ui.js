    ...
    function showRoutes() {
      ...
      routing.result.routes.forEach((route, i) => {
        ...
        const save = element('button', `Save Route ${i + 1} GPX ↓`, 'route-select'); save.addEventListener('click', () => { download(new Blob([plannedGPX(route)], { type: 'application/gpx+xml;charset=utf-8' }), `trailplaner-route-${i + 1}-PROVISIONAL.gpx`); track('gpx_exported'); });
    ...
    function allRoutesGPX(routes) {
      ...
      return `<?xml version="1.0" encoding="UTF-8"?>\n<gpx version="1.1" creator="trailplaner" xmlns="http://www.topografix.com/GPX/1/1"><metadata><name>trailplaner route options</name><desc>${xmlText(warning)}</desc><time>${stamp}</time></metadata>${waypoints}${tracksXML}</gpx>`;
    }
