    ...
    $('find-routes').addEventListener('click', async () => {
      ...
      $('map-route-toolbar').hidden = true; $('map-route-dots').replaceChildren(); $('map-export-action').disabled = true;
      toast('Finding routes in pin order first. Transport: 1 km first, extending up to 20 km if needed…', false, 0);
      ...
      try {
        ...
        try {
          $('routing-status').textContent = 'Checking mapped paths and transport: within 1 km first, then up to 20 km if needed…'; toast($('routing-status').textContent, false, 0);
          ...
          try { result = await workerPlan(mapData.data, points, settings, official.features, serial); }
            ...
            toast(settings.loop ? 'No shared loop stop within a 1 km walk of both end pins. Checking mapped approaches up to 20 km…' : 'No transport within a 1 km walk at an end. Checking connected approaches up to 20 km…', false, 0);
            ...
            if (serial !== routing.serial) return;
            controller.signal.throwIfAborted();
            result = await workerPlan(mapData.data, points, { ...settings, radius: TrailRouter.MAX_APPROACH, maxApproach: TrailRouter.MAX_APPROACH }, official.features, serial);
