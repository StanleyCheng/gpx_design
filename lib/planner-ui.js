    ...
    function routeChecksPassed() { return $('confirm-waypoints').checked && ['check-transit', 'check-access', 'check-offsets'].every(id => $(id).checked); }
    function resetRouteChecks() { for (const id of ['check-transit', 'check-access', 'check-offsets']) $(id).checked = false; }
    ...
    function invalidateRoutes() {
      stopRouting(); routing.selected = null; routing.result = null; resetRouteChecks(); $('confirm-waypoints').checked = false;
      $('route-options').replaceChildren(element('div', 'Confirm your places, then search for up to three distinct walking connections. Every option must include all your waypoints and mapped transport services at both ends.', 'routes-empty'));
    ...
    function updateGuidance() {
      const n = state.points.length, valid = n > 0 && n <= 16 && (!state.requiresTraceReview || (trace.reviewed && $('confirm-trace').checked));
      $('find-routes').disabled = !valid || !$('confirm-waypoints').checked || routing.busy;
      $('find-routes').textContent = routing.busy ? 'Finding walking connections…' : 'Find up to 3 routes →';
      $('view-inputs').disabled = !n || !map;
      const next = routing.selected ? 'Next: compare the route, check both transport services and trail access, then save GPX below.' : routing.result ? 'Next: choose a route to see its complete path and the checks needed before GPX export.' : !n ? 'Next: add the places your hike must visit, using coordinates, a file, an image or map pins.' : n > 16 ? 'This planner supports up to 16 mandatory places per local hike. No places will be silently dropped.' : 'Next: inspect every numbered pin, confirm the places below, then find routes.';
      ...
      if (routing.selected) {
        $('save-gpx').disabled = !routeChecksPassed(); $('save-gpx').textContent = 'Save selected route as GPX ↓';
        $('route-export-status').textContent = routeChecksPassed() ? 'Ready to save this provisional route in the direction shown. Your checks are not a safety certification.' : 'Complete all three checks to enable the selected route’s GPX export.';
    ...
    function showRoutes() {
      ...
      routing.result.routes.forEach((route, i) => {
        ...
        const select = element('button', routing.selected?.id === route.id ? 'Viewing this route ✓' : 'View route & checks →', 'route-select primary'); select.addEventListener('click', () => selectRoute(route)); card.append(facts, select); cards.append(card);
      });
    ...
    function selectRoute(route) {
      routing.selected = route; resetRouteChecks(); showRoutes(); routeDetails(); render(); $('selected-route-detail').scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
    ...
    function paintPlannedRoute() {
      ...
      if (map) {
        ...
        state.points.forEach((p, i) => L.marker([p.lat, p.lon], { icon: L.divIcon({ className: 'number-pin', html: `<span>${i + 1}</span>`, iconSize: [33, 33], iconAnchor: [16, 16] }), title: p.name }).bindPopup(element('div', `${p.name} · confirmed input; offset ${Math.ceil(r.snaps[i].metres)} m`)).addTo(markers));
      ...
      $('export-note').textContent = 'The solid route follows connected OSM walking geometry. Small offsets to input pins and transport stops remain unverified and are not filled with invented connectors. Complete the three checks above before GPX export.';
    }
    ...
    $('find-routes').addEventListener('click', async () => {
      ...
      stopRouting(); const serial = routing.serial, controller = new AbortController(); routing.controller = controller; routing.busy = true; routing.selected = null; routing.result = null; resetRouteChecks(); $('selected-route-detail').hidden = true; $('route-export-review').hidden = true; clearExport();
    ...
    $('cancel-routing').addEventListener('click', () => { stopRouting(); $('routing-status').textContent = 'Search cancelled. Your waypoints are unchanged.'; $('route-options').replaceChildren(element('div', 'Search cancelled. Confirm your places and search again when ready.', 'routes-empty')); updateGuidance(); });
    $('confirm-waypoints').addEventListener('change', updateGuidance);
    $('view-inputs').addEventListener('click', () => { routing.selected = null; resetRouteChecks(); $('selected-route-detail').hidden = true; $('route-export-review').hidden = true; if (routing.result) showRoutes(); render(); $('map-title').scrollIntoView({ behavior: 'smooth' }); });
    for (const id of planFields) $('plan-' + id).addEventListener('change', () => render(false));
    for (const id of ['check-transit', 'check-access', 'check-offsets']) $(id).addEventListener('change', () => { clearExport(); updateGuidance(); });
