    ...
    function allRoutesGPX(routes) {
      ...
      const tracksXML = routes.map((r, i) => { const option = Number(String(r.id).split('-').at(-1)) || i + 1, fordWarning = r.fordCrossings?.length ? ` CAUTION: ${r.fordCrossings.length} mapped ford crossing${r.fordCrossings.length === 1 ? '' : 's'} on an AFCD trail corridor; check official notices, recent rain and water level.` : ''; return `<trk><name>${xmlText(`Route ${option} — ${r.title} — provisional`)}</name><desc>${xmlText(`${warning}${fordWarning} Start near ${r.start.name} (${km(r.start.approach)} approach; ${Math.ceil(r.start.accessGap)} m unrouted stop gap); end near ${r.end.name} (${km(r.end.approach)} exit; ${Math.ceil(r.end.accessGap)} m unrouted stop gap). Visit order ${r.order.map(index => index + 1).join(" → ")}.`)}</desc><trkseg>${r.coords.map(p => `<trkpt lat="${decimal(p.lat)}" lon="${decimal(p.lon)}"></trkpt>`).join('')}</trkseg></trk>`; }).join('');
