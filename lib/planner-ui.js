    ...
    const planFields = ['region', 'order', 'loop', 'date', 'distance', 'radius', 'tolerance', 'roads', 'provider'];
    const providers = {
    ...
    const loopEnabled = () => $('plan-loop').getAttribute('aria-checked') === 'true';
    const routingFingerprint = () => JSON.stringify([state.points, state.segments.length, state.source, ...planFields.map(id => id === 'loop' ? loopEnabled() : $('plan-' + id).value)]);
    ...
    function updateGuidance() {
      ...
      $('map-route-action').textContent = 'Find';
    ...
    function showRoutes() {
      ...
      routing.result.routes.forEach((route, i) => {
        ...
        if (route.start.extendedApproach || route.end.extendedApproach) facts.append(element('li', `Extended transport walk: ${[route.start.extendedApproach ? 'start' : '', route.end.extendedApproach ? 'finish' : ''].filter(Boolean).join(' and ')}. ${route.loop ? 'No shared loop stop met the 1 km limit for both approaches' : 'No qualifying 1 km approach was found at that end'}; the 10 km limit applies.`));
    ...
    function paintPlannedRoute() {
      ...
      if (map) {
        routes.forEach((route, i) => {
          ...
          if (midpoint) L.marker([midpoint.lat, midpoint.lon], { interactive: false, icon: L.divIcon({ className: 'route-line-label', html: `<span style="background:${color}">Route ${i + 1} · ${km(route.metres)}</span>`, iconSize: [122, 26], iconAnchor: [61, -10] }) }).addTo(markers);
    ...
    async function boundedJSON(url, options, maxBytes = 25 * 1024 * 1024) {
      ...
      try { while (true) { const { done, value } = await reader.read(); if (done) break; size += value.length; if (size > maxBytes) throw new Error('Map response is too large. Reduce the search radius.'); chunks.push(value); } } catch (e) { await reader.cancel().catch(() => {}); throw e; }
    ...
    async function getMapData(box, selected, signal, areas = []) {
    ...
    async function getBackendPlan(points, settings, provider, region, signal) {
      let response;
      try {
        response = await fetch(ROUTE_BACKEND_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ points, settings, provider, region }), signal: AbortSignal.any([signal, AbortSignal.timeout(235000)]) });
      } catch (error) {
        if (signal.aborted) throw error;
        const timedOut = ['TimeoutError', 'AbortError'].includes(error?.name);
        const failure = new Error(timedOut ? 'The route backend took too long to respond. Your waypoints are unchanged; please retry.' : 'The route backend could not be reached.');
        failure.directFallback = !timedOut; throw failure;
      }
      let payload = {};
    ...
    $('find-routes').addEventListener('click', async () => {
      ...
      $('map-route-toolbar').hidden = true; $('map-route-dots').replaceChildren(); $('map-export-action').disabled = true;
      toast('Finding routes in pin order first. Transport: 1 km first, extending up to 10 km if needed…', false, 0);
      $('route-options').replaceChildren(element('div', 'Checking real paths and transport connections. The search may take up to four minutes if a transport extension is needed; you can cancel.', 'routes-empty')); render(false); setStage('routes', { expand: false });
      ...
      try {
        ...
        const settings = { radius, maxApproach: 1000, maxDistance: +$('plan-distance').value, maxRoad: +$('plan-roads').value, tolerance: +$('plan-tolerance').value, optimize: $('plan-order').value === 'optimize', loop: loopEnabled() };
        const points = state.points.map(p => ({ ...p }));
        try {
          $('routing-status').textContent = 'Checking mapped paths and transport: within 1 km first, then up to 10 km if needed…'; toast($('routing-status').textContent, false, 0);
          ...
          let mapData = await getMapData(box, $('plan-provider').value, controller.signal);
          ...
          try { result = await workerPlan(mapData.data, points, settings, official.features, serial); }
            ...
            const areas = TrailRouter.transportExpansion(points, error);
            if (!areas.length) throw error;
            toast(settings.loop ? 'No shared loop stop within a 1 km walk of both end pins. Checking mapped approaches up to 10 km…' : 'No transport within a 1 km walk at an end. Checking connected approaches up to 10 km…', false, 0);
            ...
            if (serial !== routing.serial) return;
            controller.signal.throwIfAborted();
            result = await workerPlan(mapData.data, points, { ...settings, radius: 10000, maxApproach: 10000 }, official.features, serial);
    ...
    $('plan-loop').addEventListener('click', () => {
      ...
      $('plan-loop').setAttribute('aria-checked', String(enabled));
      $('plan-loop').querySelector('.switch-state').textContent = enabled ? 'On' : 'Off';
      $('plan-loop').dispatchEvent(new Event('change'));
    });
    ...
    function reversePlannedRoute() {
      ...
      r.reason = r.reversed ? 'Reversed by you. Check the new visit order, arrival and departure below.' : r.originalReason;
      showRoutes(); routeDetails(); render(); toast('Route reversed. Review arrival, departure and trail access again before saving.');
    }
    function plannedGPX(r) {
      const warning = `PROVISIONAL ROUTE — independently check before navigation. Connected OSM walking ways only; no artificial connectors. Start near ${r.start.name} (${km(r.start.approach)} approach, ${Math.ceil(r.start.accessGap)} m unrouted stop gap); end near ${r.end.name} (${km(r.end.approach)} exit, ${Math.ceil(r.end.accessGap)} m gap). Transport approaches prefer 1 km, extending up to 10 km only when needed. Visit order: ${r.order.map(i => i + 1).join(" → ")}. All ${state.points.length} mandatory waypoints visited within the explicitly accepted ${$('plan-tolerance').value} m tolerance. OSM snapshot ${r.osmTimestamp || 'unknown'}. Hiking date ${$('plan-date').value}. Timetables, closures, permits, terrain and management are not automatically verified. OpenStreetMap contributors, ODbL: https://www.openstreetmap.org/copyright. AFCD corridor matches, where available, are approximate. No elevation invented.`;
    ...
    function allRoutesGPX(routes) {
      ...
      const tracksXML = routes.map((r, i) => { const option = Number(String(r.id).split('-').at(-1)) || i + 1; return `<trk><name>${xmlText(`Route ${option} — ${r.title} — provisional`)}</name><desc>${xmlText(`${warning} Start near ${r.start.name} (${km(r.start.approach)} approach; ${Math.ceil(r.start.accessGap)} m unrouted stop gap); end near ${r.end.name} (${km(r.end.approach)} exit; ${Math.ceil(r.end.accessGap)} m unrouted stop gap). Visit order ${r.order.map(index => index + 1).join(" → ")}.`)}</desc><trkseg>${r.coords.map(p => `<trkpt lat="${decimal(p.lat)}" lon="${decimal(p.lon)}"></trkpt>`).join('')}</trkseg></trk>`; }).join('');
