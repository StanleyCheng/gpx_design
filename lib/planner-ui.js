    ...
    async function getMapData(box, selected, signal) {
      ...
      for (const choice of choices) {
        const provider = providers[choice], entry = provider && routing.cache.get(provider.url + '/' + box.join(','));
    ...
    $('find-routes').addEventListener('click', async () => {
      ...
      $('route-options').replaceChildren(element('div', 'Checking real paths and transport connections. The search may take up to two minutes; you can cancel.', 'routes-empty')); render(false); setStage('routes', { expand: false });
      ...
      try {
        ...
        try {
          ...
          const result = await workerPlan(mapData.data, points, settings, official.features, serial);
          if (serial !== routing.serial) return;
          controller.signal.throwIfAborted();
