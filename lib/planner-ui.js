    ...
    const ROUTE_RUNNER_COLOR = '#ff6f7d';
    const DIFFICULTY_STYLES = [
      { color: '#777777', label: 'Unknown difficulty', rank: 0 },
      { color: '#13834b', label: 'Easy hiking', rank: 1 },
      { color: '#e34e9b', label: 'More demanding hiking', rank: 2 },
      { color: '#d00000', label: 'Difficult / alpine hiking', rank: 3 }
    ];
    ...
    function routeDifficultySections(route) {
      ...
      for (let i = 0; i + 1 < route.coords.length; i++) {
        ...
        const style = DIFFICULTY_STYLES[grade < 0 ? 0 : grade <= 1 ? 1 : grade <= 3 ? 2 : 3];
    ...
    function startRouteRun(routes) {
      ...
      const runners = routes.flatMap(route => {
        ...
        const dot = L.circleMarker([geometry.coords[0].lat, geometry.coords[0].lon], { radius: width / 2, stroke: false, fillColor: ROUTE_RUNNER_COLOR, fillOpacity: 1, interactive: false }).addTo(markers);
        routeRunAnimation.dots.push(dot);
        return [{ dot, geometry, duration: Math.max(8000, Math.min(24000, geometry.total * 9)) }];
      });
      ...
      return new Promise((resolve, reject) => {
        ...
        try {
          ...
          worker.onmessage = e => {
            ...
            if (e.data.progress) { $('routing-status').textContent = e.data.progress; toast(e.data.progress, false, 0); }
            else {
              cleanup();
    ...
    $('find-routes').addEventListener('click', async () => {
      ...
      try {
        ...
        try {
          ...
          $('routing-status').textContent = 'Using the local route engine. Downloading paths from map providers…'; toast($('routing-status').textContent, false, 0);
