    ...
    function updateRecognition() {
      ...
      $('use-ai-waypoints').disabled = !ai.candidates.some(p => p.included) || !$('ai-complete').checked || !ai.candidates.filter(p => p.included).every(p => p.confirmed && p.coordinate);
    }
    ...
    function displayRecognition(result) {
      ...
      ai.candidates = result.waypoints.map(p => ({ ...p, included: false, confirmed: false, coordinate: null })); const rows = document.createDocumentFragment();
      ai.candidates.forEach((p, i) => {
        ...
        const includeLabel = element('label', undefined, 'image-confirm'), include = document.createElement('input'); include.type = 'checkbox'; includeLabel.append(include, element('span', 'Include this as a mandatory place.')); const checkLabel = element('label', undefined, 'image-confirm'), check = document.createElement('input'); check.type = 'checkbox'; checkLabel.append(check, element('span', 'I independently checked this coordinate and image marker.'));
        ...
        const validate = () => { p.coordinate = null; try { const points = parseCoordinates(input.value); if (points.length === 1) p.coordinate = { ...points[0], name: p.label }; } catch {} check.disabled = !p.included || !p.coordinate; mapLink.hidden = !p.coordinate; if (p.coordinate) mapLink.href = `https://www.openstreetmap.org/?mlat=${p.coordinate.lat}&mlon=${p.coordinate.lon}#map=16/${p.coordinate.lat}/${p.coordinate.lon}`; p.confirmed = p.included && check.checked && !!p.coordinate; updateRecognition(); };
        include.addEventListener('change', () => { p.included = include.checked; if (!p.included) check.checked = false; $('ai-complete').checked = false; validate(); }); input.addEventListener('input', () => { check.checked = false; $('ai-complete').checked = false; validate(); }); check.addEventListener('change', validate);
        row.append(label, includeLabel, input, status, mapLink, checkLabel); rows.append(row); validate();
      });
    ...
    $('use-ai-waypoints').addEventListener('click', () => {
      if ($('use-ai-waypoints').disabled || !ai.candidates.some(p => p.included) || !ai.candidates.filter(p => p.included).every(p => p.confirmed && p.coordinate)) return;
      replaceDraft(ai.candidates.filter(p => p.included).map(p => ({ ...p.coordinate })), [], 'User-confirmed Kimi image waypoints'); message('Confirmed image places added. Review the pins and choose your route settings in step 2.'); $('route-builder').scrollIntoView({ behavior: 'smooth' });
    });
    // END RECOGNITION UI
