    ...
    function displayRecognition(result) {
      ...
      ai.candidates.forEach((p, i) => {
        ...
        const status = element('p', `${p.basis.replaceAll('_', ' ')} · ${p.confidence} confidence. ${p.evidence}`, 'help'), coordinateStatus = element('p', '', 'help error'); coordinateStatus.setAttribute('aria-live', 'polite'); coordinateStatus.hidden = true; const mapLink = element('a', 'Check coordinate on OpenStreetMap ↗', 'text-button'); mapLink.target = '_blank'; mapLink.rel = 'noopener noreferrer';
        const validate = () => { p.coordinate = null; try { const points = parseCoordinates(input.value); if (points.length === 1) p.coordinate = { ...points[0], name: p.label }; } catch {} coordinateStatus.hidden = !!p.coordinate || (!p.included && !input.value.trim()); coordinateStatus.textContent = input.value.trim() ? 'Use one location like 22.36, 114.37.' : 'Add one checked latitude and longitude for this included place.'; mapLink.hidden = !p.coordinate; if (p.coordinate) mapLink.href = `https://www.openstreetmap.org/?mlat=${p.coordinate.lat}&mlon=${p.coordinate.lon}#map=16/${p.coordinate.lat}/${p.coordinate.lon}`; updateRecognition(); };
    ...
    $('recognize-map').addEventListener('click', async () => {
      ...
      try {
        ...
        if (!body.result || !Array.isArray(body.result.waypoints) || body.result.waypoints.length > MAX_WAYPOINTS || !body.result.area || !Array.isArray(body.result.warnings) || !Array.isArray(body.result.questions)) throw new Error('The private server returned an invalid result. No waypoints were accepted.');
