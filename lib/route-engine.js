...
(function (root, factory) {
  ...
  const PREFERRED_APPROACH = 1000, MAX_APPROACH = 10000;
  ...
  function buildGraph(data, features = []) {
    ...
    for (const way of elements.filter(e => e.type === 'way' && e.tags?.highway)) {
      ...
      for (let i = 1; i < (way.nodes || []).length; i++) {
        ...
        const edge = { key: edgeKey(a.id, b.id), segment, way: way.id, metres, road: ROAD.has(tags.highway), steps: tags.highway === 'steps', hiking: hiking.get(way.id) || [], official: corridor(a, b), unknown: !ROAD.has(tags.highway) && !tags.sac_scale, clear: ['excellent', 'good'].includes(tags.trail_visibility), name: nameOf(way) };
    ...
    return { nodes: eligible, allNodes: nodes, adj, reverse, segments, relations, features, excludedWays, timestamp: data.osm3s?.timestamp_osm_base || null };
  }
  ...
  function snapWaypoints(graph, points, maxMetres) {
    const snaps = points.map((point, index) => {
      ...
      for (const segment of graph.segments) {
        ...
        if (projected.metres <= maxMetres && (!match || projected.metres < match.metres)) match = { ...projected, segment, index, original: point };
      }
      if (!match) throw routeError('WAYPOINT_OFF_PATH', `Waypoint ${index + 1} has no eligible path within ${maxMetres} m. The path may be absent from the downloaded data or excluded by access, terrain or visibility tags. Move or correct that waypoint, or explicitly change the tolerance; it will not be skipped.`, { waypoint: index });
      if (match.ratio <= 1e-8) return { id: match.segment.a, point: { lat: graph.nodes.get(match.segment.a).lat, lon: graph.nodes.get(match.segment.a).lon }, metres: match.metres, index, original: point };
      if (match.ratio >= 1 - 1e-8) return { id: match.segment.b, point: { lat: graph.nodes.get(match.segment.b).lat, lon: graph.nodes.get(match.segment.b).lon }, metres: match.metres, index, original: point };
      return match;
    });
    const groups = new Map();
    for (const snap of snaps) if (!snap.id) { if (!groups.has(snap.segment.key)) groups.set(snap.segment.key, []); groups.get(snap.segment.key).push(snap); }
    for (const group of groups.values()) {
      ...
      for (const snap of group.sort((x, y) => x.ratio - y.ratio)) {
        const prior = sequence.at(-1);
        if (distance(prior.point, snap.point) < .05) snap.id = prior.id;
        else { snap.id = `waypoint:${snap.index}`; graph.nodes.set(snap.id, { type: 'node', id: snap.id, ...snap.point, virtual: true }); sequence.push({ id: snap.id, point: snap.point }); }
      }
    ...
    return snaps.map(({ segment, ratio, ...snap }) => snap);
  }
  ...
  function endpoints(tree, stops, role, maxApproach) {
    ...
    return (nearby.length ? nearby : reachable).map(stop => {
    ...
    }).sort((a, b) => a.path.metres - b.path.metres || a.convenience - b.convenience).slice(0, 12);
  }
  ...
  function plan(data, points, inputSettings = {}, features = [], progress = () => {}) {
    const settings = { tolerance: 30, radius: 1000, maxApproach: 1000, maxDistance: 30000, maxRoad: 1500, optimize: true, ...inputSettings };
    ...
    progress('Snapping every waypoint onto eligible mapped paths…');
    ...
    for (const [variant, spec] of variants.entries()) {
      ...
      const order = spec.reorder ? optimizeOrder(matrix, arrivals, departures) : ordered;
      ...
      const fresh = [];
      for (const start of arrivals[first]) for (const end of departures[last]) {
    ...
    if (!selected.length) {
      ...
      throw routeError('ROUTE_LIMITS', `Connected paths were found, but none met your limits: ${limitReasons.join('; ') || 'no usable transport-to-transport walk through every pin was found'}. No limits were relaxed.`);
    }
    const notices = [];
  ...
  function boundingBox(points, radius) {
    ...
    if (box[1] < -180 || box[3] > 180 || box[3] - box[1] > 180 || area > 250e6) throw routeError('AREA_TOO_LARGE', 'The core waypoint area exceeds 250 km² or crosses the date line. Use closer waypoints or split this into separate hikes. The 10 km transport extension does not remove this core-area limit.');
    return box.map(n => Number(n.toFixed(6)));
  }
  function queryFor(box, areas = []) {
    if (areas.length > 2 || areas.some(p => !validPoint(p) || p.radius !== MAX_APPROACH)) throw new Error('Transport expansion must use at most two valid 10 km endpoint areas.');
  ...
  return { distance, walkable, passableNode, buildGraph, nearest, snapWaypoints, transportStops, search, pathFrom, distinct, optimizeOrder, plan, boundingBox, queryFor, transportExpansion, coverageBox };
});
