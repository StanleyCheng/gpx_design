...
(function (root, factory) {
  ...
  const ROUTING_POLICY_VERSION = 1;
  ...
  function buildGraph(data, features = [], options = {}) {
    ...
    if (!eligible.size) throw new Error('No eligible walking network was found under your access and terrain settings.');
    return { nodes: eligible, allNodes: nodes, adj, reverse, segments, relations, features, excludedWays, excludedOfficialFords: [...excludedOfficialFords], timestamp: data.osm3s?.timestamp_osm_base || null, check: options.check };
  }
  ...
  // Tolerance is a hard eligibility limit. Once inside it, minimize walking
  // distance first; proximity and stable source identity only break ties.
  const compareSnapScore = (a, b) => a.routeMetres - b.routeMetres || a.snapMetres - b.snapMetres || compareText(a.tie, b.tie);
  ...
  function plan(data, points, inputSettings = {}, features = [], progress = () => {}, control = {}) {
    ...
    for (const key of ['tolerance', 'radius', 'maxApproach', 'maxDistance', 'maxRoad']) {
      if (!Number.isFinite(settings[key]) || settings[key] < (key === 'maxRoad' ? 0 : Number.MIN_VALUE)) throw routeError('INVALID_SETTINGS', `Choose a finite positive ${key}${key === 'maxRoad' ? ' (zero is allowed)' : ''}.`);
    }
    if (typeof settings.optimize !== 'boolean') throw routeError('INVALID_SETTINGS', 'Choose a valid waypoint order setting.');
    ...
    if (typeof settings.allowHarderHiking !== 'boolean') throw routeError('INVALID_SETTINGS', 'Choose a valid harder hiking paths setting: on or off.');
    ...
    if (selected.some(route => route.harderTerrainMetres > 0)) notices.push('Some sections are tagged harder than hiking (T1), including mountain or alpine terrain. Review each section’s difficulty before using the route. Untagged paths have unknown difficulty.');
    ...
    check();
    return { policyVersion: ROUTING_POLICY_VERSION, routes: selected, notices, eligibleNodes: graph.nodes.size, excludedWays: graph.excludedWays, stopCount: transit.stops.length, unlinkedStops: transit.unlinked, timestamp: graph.timestamp, qualified: 'provisional', settings };
  }
