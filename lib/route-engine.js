...
(function (root, factory) {
  ...
  function summarize(graph, chain, start, end, snaps, order, settings, failures) {
    ...
    return {
      ...
      snaps, order, loop: settings.loop, relations, official: officialIds.map(i => graph.features[i].properties),
  ...
  function plan(data, points, inputSettings = {}, features = [], progress = () => {}) {
    const settings = { tolerance: 30, radius: 1000, maxApproach: 1000, maxDistance: 30000, maxRoad: 1500, optimize: true, loop: false, ...inputSettings };
  ...
  return { MAX_WAYPOINTS, distance, walkable, passableNode, buildGraph, nearest, snapWaypoints, transportStops, search, pathFrom, distinct, optimizeOrder, plan, boundingBox, queryFor, transportExpansion, coverageBox };
});
