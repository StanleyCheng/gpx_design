...
(function (root, factory) {
  ...
  function connectedLoopSnapCombination(graph, candidateSets) {
    const componentOf = strongComponents(graph), firstComponents = new Set(candidateSets[0].map(candidate => componentOf.get(candidate.id)));
    const shared = [...firstComponents].filter(component => candidateSets.every(candidates => candidates.some(candidate => componentOf.get(candidate.id) === component)));
    const ranked = shared.map(component => {
      const candidates = candidateSets.map(set => set.filter(candidate => componentOf.get(candidate.id) === component));
      const snapMetres = candidates.reduce((sum, set) => sum + set[0].metres, 0);
      return { component, candidates, snapMetres, tie: candidates.map(set => set[0].tie).join('>') };
    }).sort((a, b) => a.snapMetres - b.snapMetres || compareText(a.tie, b.tie));
    if (!ranked.length) return null;
    const bestSnap = ranked[0].snapMetres, options = [];
    for (const option of ranked) {
      if (Math.abs(option.snapMetres - bestSnap) > 1e-9) break;
      const connected = connectedSnapCombination(graph, option.candidates);
      if (!connected.snaps) continue;
      const closingTree = search(graph, connected.snaps.at(-1).id, 'distance', new Map(), false, [connected.snaps[0].id]);
      const closing = pathFrom(closingTree, connected.snaps[0].id);
      if (closing) options.push({ snaps: connected.snaps, snapMetres: connected.score.snapMetres, routeMetres: connected.score.routeMetres + closing.metres, tie: connected.score.tie });
    }
    return options.sort(compareSnapScore)[0]?.snaps || null;
  }
  function snapWaypoints(graph, points, maxMetres, loop = false) {
    ...
    const connected = candidateSets.length > 1 ? (loop ? connectedLoopSnapCombination(graph, candidateSets) : connectedSnapCombination(graph, candidateSets).snaps) : [candidateSets[0][0]];
  ...
  function summarize(graph, chain, start, end, snaps, order, settings, failures) {
    ...
    return {
      ...
      reversible: chain.ids.slice(1).every((id, i) => (graph.adj.get(id) || []).some(e => e.to === chain.ids[i])) && (settings.loop || (end.stop.services.some(s => s.alight) && start.stop.services.some(s => s.board))),
  ...
  function loopEndpoints(snaps, shortest, canReorder) {
    ...
    for (let i = canReorder ? 0 : snaps.length - 1; i < snaps.length; i++) {
      const path = pathFrom(shortest[i], snaps[0].id);
  ...
  function plan(data, points, inputSettings = {}, features = [], progress = () => {}) {
    const settings = { tolerance: 30, radius: 1000, maxApproach: 1000, maxDistance: 50000, maxRoad: 80000, optimize: true, loop: false, allowOfficialFords: false, ...inputSettings };
    ...
    const snaps = snapWaypoints(graph, points, settings.tolerance, settings.loop), n = snaps.length;
    ...
    for (let i = 0; i < n; i++) {
      ...
      const targets = [...routeTargets, ...(!settings.loop && (canReorder || i === n - 1) ? stopIds : [])];
    ...
    if (!orderedFailure && missingLoop) {
      orderedFailure = routeError('DISCONNECTED_WAYPOINTS', `Waypoint ${n} cannot return to waypoint 1 over eligible directed paths, so the loop cannot close in pin order. Missing mapped connections, foot one-way restrictions, access restrictions or excluded terrain may separate them. No closing line was invented.`);
    ...
    selected.forEach((route, i) => {
      ...
      route.id = `route-${i + 1}`;
      if (settings.loop) { route.title += ' · loop'; route.reason += ' Returns over mapped paths to waypoint 1, the exact route start/finish; transport is not required for a loop and some paths may be retraced.'; }
    });
  ...
  return { MAX_WAYPOINTS, MAX_SNAP_CANDIDATES, MAX_MAP_ELEMENTS, MAX_APPROACH, TRANSPORT_EXPANSION_STEPS, distance, walkable, passableNode, buildGraph, nearest, snapWaypoints, transportStops, search, pathFrom, distinct, optimizeOrder, plan, boundingBox, queryFor, transportExpansion, coverageBox };
});
