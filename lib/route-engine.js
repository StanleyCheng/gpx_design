...
(function (root, factory) {
  ...
  function buildGraph(data, features = []) {
    ...
    for (const way of elements.filter(e => e.type === 'way' && e.tags?.highway)) {
      ...
      for (let i = 1; i < (way.nodes || []).length; i++) {
        ...
        const edge = { key: edgeKey(a.id, b.id), segment, way: way.id, metres, road: ROAD.has(tags.highway), steps: tags.highway === 'steps', hiking: hiking.get(way.id) || [], official: corridor(a, b), unknown: !ROAD.has(tags.highway) && !tags.sac_scale, name: nameOf(way) };
  ...
  function snapWaypoints(graph, points, maxMetres) {
    const snaps = points.map((point, index) => {
      ...
      if (!match) throw new Error(`Waypoint ${index + 1} has no eligible path within ${maxMetres} m. Move or correct that waypoint, or explicitly change the tolerance; it will not be skipped.`);
  ...
  function plan(data, points, inputSettings = {}, features = [], progress = () => {}) {
    ...
    const snaps = snapWaypoints(graph, points, settings.tolerance);
