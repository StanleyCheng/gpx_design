...
export function normalizeRecognition(raw) {
  if (!raw || !['located', 'needs_context', 'not_map'].includes(raw.status) || !Array.isArray(raw.waypoints) || raw.waypoints.length > 16) throw new Error('The recognition response was invalid. No coordinates were accepted.');
...
export async function kimiRecognize(input, key) {
  const response = await fetch('https://api.moonshot.ai/v1/chat/completions', { method: 'POST', signal: AbortSignal.timeout(110000), headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` }, body: JSON.stringify({ model: 'kimi-k3', response_format: { type: 'json_object' }, max_tokens: 8192, messages: [{ role: 'system', content: PROMPT }, { role: 'user', content: [{ type: 'text', text: `Map context supplied by user (may be wrong): ${input.context || 'none'}. Locate the map and visible marked waypoints, retaining uncertainty.` }, { type: 'image_url', image_url: { url: input.image } }] }] }) });
